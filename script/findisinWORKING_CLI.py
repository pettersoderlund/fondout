...
"""
    ...
    TODO: 
        ...
        - Statisik över resultat? Case 1, 2, 3 etc.
        - Bryt ut lösenord och användarnamn till ENV_VAR
        - Lägg till en timer
        - Bestäm funktionalitet - gör argument options för cli
"""
...
def execute_share_search_query(connection, query, name): 
...
def find_by_share_exact_name(connection, name):
    query = ("SELECT s.name, s.isin FROM share s "
        "WHERE s.name = (%s) "
        + QUERY_WHERE_AND + QUERY_ORDER +
        "LIMIT 1")
    return execute_share_search_query(connection, query, name)

def find_by_share_exact_alias(connection, name):
...
def find_by_share_fuzzy_alias(connection, name):
    query = ("SELECT sa.name, s.isin "
        "FROM share s "
        "JOIN share_alias sa on sa.share = s.id "
...
def find_by_share_reverse_fuzzy_alias(connection, name):
    query = ("SELECT sa.name, s.isin "
        "FROM share s "
        "JOIN share_alias sa on sa.share = s.id "
...
def share_company_by_isin(connection, isin):
    cursor = connection.cursor(buffered=True)
    ...
    if (cursor is not None):
        share_company_name = cursor.fetchone()
    else:
        share_company_name = None

    cursor.close()

    if (share_company_name is not None):
        share_company_name = share_company_name[0]

    return share_company_name

def find_exact_share_routine(name, connection): 
    share = None

    share = find_by_share_exact_name(connection, name)
    if (share is None):
        share = find_by_share_exact_alias(connection, name)
    if (share is None):
        share = find_by_share_company_exact_name(connection, name)

    return share

def find_share_routine(name, connection):
    share = None

    share = find_exact_share_routine(name, connection)

    # Do not allow fuzzy search on search strings shorter than four letters!
    if (len(name) > 3):
        if (share is None): 
            share = find_by_share_fuzzy_name(connection, name)
        if (share is None): 
            share = find_by_share_company_fuzzy_name(connection, name)
        if (share is None): 
            share = find_by_share_fuzzy_alias(connection, name)
        if (share is None): 
            share = find_by_share_reverse_fuzzy_alias(connection, name)
        if (share is None): 
            share = find_by_share_reverse_fuzzy_name(connection, name)
        if (share is None): 
...
# --------------------------------------------------------------------------
QUERY_WHERE_AND = ( "AND s.isin IS NOT NULL "
                    "AND (s.category = 1 or s.category is null) ")
QUERY_ORDER     = "ORDER BY s.category desc, s.id asc "
...
if __name__ == "__main__":
    ...
    for (share_name,) in unidentifiedShares:
        ...
        found_share = find_exact_share_routine(share_name, cnx)
        ...
        if (found_share is None):
            found_share = find_share_routine(share_name, cnx)
        ...
        if (found_share is None):
            ...
            for suffix in company_suffix:
                ...
                if (new_name not in used_names):
                    found_share = find_share_routine(new_name, cnx)
        ...
        found_share_company = ""
        if(found_isin != ""):
            found_share_company = share_company_by_isin(cnx, found_isin)
